...
"""Dimitri BaseDyGraph-V2 with continuous candle input and direct price output.

This is the controlled continuous-input counterpart of the native-s1
token-to-price diagnostic.  It keeps the exact x0jhc0tx four-block dual-fusion
architecture, graph prior, optimisation protocol and dense teacher-forced
one-step price objective.  The only representation change is that the
pre-tokenisation, context-normalised OHLCVA sequence is projected directly into
D=96 instead of being quantised to a coarse token and looked up in an embedding
table.

Checkpoint selection uses one-minute cumulative-log-change MAE at the forecast
origin on the selected test split. The run is deliberately test-selected and
...
def _training_epoch(
    *,
    ...
    for batch in tqdm(loader, desc="train", leave=False, dynamic_ncols=True):
        continuous_values = torch.as_tensor(batch["continuous_values"]).to(
            device=device,
            dtype=torch.float32,
            non_blocking=True,
        )
        ...
        output = model(continuous_values)
        ...
        if not torch.isfinite(objective):
            raise FloatingPointError("Non-finite dense continuous-price objective.")
        objective.backward()
        optimizer.step()
...
def _evaluate(
    *,
    ...
    prediction_result = {
        "task_type": "dimitri_v2_continuous_input_direct_price_one_minute",
    ...
    if retain_graphs:
        ...
        graph_artifacts = {
            ...
            "saved_graph_time": (
                f"forecast-origin sequence position {context_length - 1}"
            ),
...
def _resolved_config(
    *,
    ...
    return {
        "model_family": "dimitri_basedygraph_v2_continuous_to_price",
        "model": {
        ...
        "data": {
            ...
            "normalisation": {
                "statistics": "first context_length rows only",
                "std_correction": 1,
                "clip": [-5.0, 5.0],
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Dimitri V2 continuous-input/direct-price one-minute diagnostic."
    )
    ...
    parser.add_argument(
        "--wandb-project",
        type=str,
        default="BaseDyGraph V2 Continuous Price TEST-CONTAMINATED",
    )
...
def main() -> None:
    ...
    _set_seed(args.seed)
    model = instantiate_dimitri_continuous_to_price_model(input_channels=6)
    observed_parameters = parameter_count(model)
    if observed_parameters != DIMITRI_CONTINUOUS_PRICE_EXPECTED_PARAMETER_COUNT:
        raise AssertionError(
            f"Continuous-to-price parameter count {observed_parameters:,} "
            f"differs from expected "
            f"{DIMITRI_CONTINUOUS_PRICE_EXPECTED_PARAMETER_COUNT:,}."
        )
    ...
    signature_payload = {
        "contract": DIMITRI_CONTINUOUS_PRICE_CONTRACT,
    ...
    metadata: dict[str, Any] = {
        ...
        "experiment_contract": DIMITRI_CONTINUOUS_PRICE_CONTRACT,
        "model_family": "dimitri_basedygraph_v2_continuous_to_price",
